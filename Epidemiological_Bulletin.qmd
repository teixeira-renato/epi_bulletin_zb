...
# Agrupando os casos por doença e gerando texto formatado
...
# Separando doenças de notificação imediata e outras
...
# Removendo qualquer espaço extra antes do título
...
table_gt <- df_summary %>%
  ...
  # Criar cabeçalhos agrupados para as semanas
  tab_spanner(
    label = "Week 34",
  ...
  # Formatar números com separador de milhar e sem decimais
  fmt_number(
    columns = c(`Suspected (Week 34)`, `Tested (Week 34)`, `Confirmed (Week 34)`,
  ...
  # Colorir os cabeçalhos das colunas agrupadas (spanners)
  tab_style(
    style = list(
  ...
  # Colorir os rótulos dos cabeçalhos das colunas
  tab_style(
    style = list(
      cell_fill(color = "orange"),
      cell_text(color = "white", weight = "bold")
  ...
  # Colorir toda a primeira coluna (doença/evento)
  tab_style(
    style = list(
      cell_fill(color = "orange"),
      cell_text(color = "white", weight = "bold")
...
grafico <- measles_lab_pad %>%
  ...
  theme_minimal() +
  theme(
    axis.text.x = element_text(angle = 90, hjust = 1, size = 6),  # Texto do eixo X
    axis.text.y = element_text(size = 6),  # Texto do eixo Y
    axis.title.x = element_text(size = 6),  # Título do eixo X
    axis.title.y = element_text(size = 6),  # Título do eixo Y
    legend.text = element_text(size = 6),  # Texto da legenda
    legend.title = element_text(size = 6, face = "bold"),  # Título da legenda
    panel.grid.major = element_line(linetype = "dotted"),
    panel.grid.minor = element_line(linetype = "dotted"),
    plot.margin = margin(10, 10, 10, 10) # Adiciona margem para manter centralizado
  )

grafico_grob <- ggplotGrob(grafico)

# 🔹 Centralizar gráfico dentro da área disponível
...
maternal_deaths_cod <- maternal_deaths_cod %>%
  mutate(cod_short_name = case_when(
    ...
    TRUE ~ `Short Name`  # Mantém os demais nomes sem alteração
  ))
...
# Criar a faixa do título (fundo verde)
...
# 🔹 Combinar tudo no layout final
grid.arrange(
  grafico_barras_grob, grafico_mapa_grob,
  ncol = 2  # Definir duas colunas para manter os gráficos lado a lado
)
